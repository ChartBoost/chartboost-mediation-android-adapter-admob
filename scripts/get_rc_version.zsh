VERSION=`grep ' android.defaultConfig.versionName =' AdMobAdapter/build.gradle.kts | sed 's/^[ \t]*android.defaultConfig.versionName.*?: //g' | sed 's/"//g'`
RELEASE_CANDIDATE="$VERSION-rc"
# Fetch from private artifactory all the versions matching RELEASE_CANDIDATE
curl -sSf -u "$JFROG_USER:$JFROG_PASS" -O 'https://cboost.jfrog.io/artifactory/private-chartboost-mediation/com/chartboost/chartboost-mediation-adapter-admob/maven-metadata.xml'
